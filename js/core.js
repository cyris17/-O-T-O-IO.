...
const core = (() => {
  ...
  const applyFallbackContent = () => {
    ...
    const lbViewed = document.getElementById('lb-viewed');
    if (lbViewed && !lbViewed.children.length) {
      lbViewed.innerHTML = '<div style="color:rgba(234,246,255,.40);font-size:.85rem;padding:14px 0;">No data yet.</div>';
    }
  };
  ...
  const checkRoute = async () => {
    ...
    if (h === '#dashboard') {
      ...
      try {
        ...
        /* Verify admin_roles membership */
        const { data: roleRow } = await supabase.from('admin_roles').select('user_id').eq('user_id', session.user.id).maybeSingle();
        if (!roleRow) {
  ...
  const login = async () => {
    ...
    if (!supabase) { if (msg) msg.innerText = 'Supabase not configured.'; return; }
    ...
    if (error) { if (msg) msg.innerText = error.message; return; }

    /* Step 2: admin_roles check */
    const { data: roleRow } = await supabase.from('admin_roles').select('user_id').eq('user_id', authData.user.id).maybeSingle();
    if (!roleRow) {
      await supabase.auth.signOut();
      if (msg) msg.innerText = 'Access denied — not an admin.';
      return;
    }

    /* Step 3: Passkey hash check */
    const { data: pkRow } = await supabase.from('site_content').select('content').eq('id', 'admin_passkey_hash').maybeSingle();
    if (pkRow?.content) {
      const inputHash = await hashString(passkey);
      if (inputHash !== pkRow.content.trim()) {
        await supabase.auth.signOut();
        if (msg) msg.innerText = 'Invalid passkey.';
        return;
      }
    }
...
/* ── App init ─────────────────────────────────────────── */
  ...
  const init = async () => {
    ...
    /* Load leaderboard after data is ready (non-blocking) */
